...
public class InstrumentViewsw extends View {
    private String color_outcircle = "#DEDEDE";
    private String color_bg_outcircle = "#2690F8";  //内环
    ...
    public InstrumentViewsw(Context context) {
        this(context, null);
    }

    public InstrumentViewsw(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public InstrumentViewsw(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawFigure(Canvas canvas, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            canvas.drawText(figure/20+"L",pointFromAngleAndRadius[0],pointFromAngleAndRadius[1],mPaint);
            canvas.restore();
        }
    }
...
//        canvas.drawText("当前进度："+progress+"%",);
        ...
        canvas.drawText("水位", centerPoint[0], baseLine1, mPaint);
...
//此时求出来的baseline可以使文字竖直居中
        ...
        canvas.drawText(progress/20 + "L", centerPoint[0], baseLine2, mPaint);
